                ...
                async with self.client_manager.get_client() as client:
                    ...
                    for alarm_obj in proc.list_alarms():
                        if alarm_obj.name == alarm:
                            # Build alarm details based on documented Alarm model
                            alarm_info = {
                                "name": alarm_obj.name,
                                "sequence_number": alarm_obj.sequence_number,
                                "trigger_time": alarm_obj.trigger_time.isoformat()
                                if alarm_obj.trigger_time
                                else None,
                                "update_time": alarm_obj.update_time.isoformat()
                                if alarm_obj.update_time
                                else None,
                                ...
                                "is_process_ok": getattr(
                                    alarm_obj, "is_process_ok", None
                                ),
                            ...
                            if (
                                hasattr(alarm_obj, "acknowledge_time")
                                and alarm_obj.acknowledge_time
                            ):
                                alarm_info["acknowledge_time"] = (
                                    alarm_obj.acknowledge_time.isoformat()
                                )
                            if hasattr(alarm_obj, "acknowledged_by"):
                                alarm_info["acknowledged_by"] = (
                                    alarm_obj.acknowledged_by
                                )
                            if hasattr(alarm_obj, "acknowledge_message"):
                                alarm_info["acknowledge_message"] = (
                                    alarm_obj.acknowledge_message
                                )
                    ...
                    return {
                        "error": True,
                        "message": (
                            f"Alarm '{alarm}' not found on processor '{processor}'"
                        ),
                        "instance": instance or self.config.instance,
                        "processor": processor,
                    }
            ...
            try:
                async with self.client_manager.get_client() as client:
                    ...
                    return {
                        ...
                        "instance": instance or self.config.instance,
                        "message": (
                            f"Alarm '{alarm}' (seq: {sequence_number}) acknowledged"
                        ),
                    }
            except Exception as e:
            ...
            try:
                async with self.client_manager.get_client() as client:
                    ...
                    return {
                        ...
                        "instance": instance or self.config.instance,
                        "message": (
                            f"Alarm '{alarm}' (seq: {sequence_number}) unshelved"
                        ),
                    }
            except Exception as e:
                ...
                async with self.client_manager.get_client() as client:
                    ...
                    for alarm in archive.list_alarms(
                        ...
                        alarm_info = {
                            "name": alarm.name,
                            "sequence_number": alarm.sequence_number,
                            "trigger_time": alarm.trigger_time.isoformat()
                            if alarm.trigger_time
                            else None,
                            "update_time": alarm.update_time.isoformat()
                            if alarm.update_time
                            else None,
                            "severity": alarm.severity,
                            "violation_count": alarm.violation_count,
                            "count": alarm.count,
                        ...
                        if alarm.is_acknowledged:
                            if (
                                hasattr(alarm, "acknowledge_time")
                                and alarm.acknowledge_time
                            ):
                                alarm_info["acknowledge_time"] = (
                                    alarm.acknowledge_time.isoformat()
                                )
                        ...
                        if instance_alarms:
                            ...
                            for proc_name, alarms in instance_alarms:
                                ...
                                for alarm in alarms:
                                    ...
                                    ack_status = (
                                        "ACK" if alarm.is_acknowledged else "UNACK"
                                    )
                                    shelved_status = (
                                        " [SHELVED]"
                                        if getattr(alarm, "is_shelved", False)
                                    ...
                                    seq_num = getattr(alarm, "sequence_number", "N/A")
                                    status_parts = [
                                        ack_status,
                                        shelved_status,
                                        ok_status,
                                    ...
                                    lines.append(
                                        f"      - {alarm.name} (seq: {seq_num}) "
                                        f"[{severity}] {status_str}"
                                    )
