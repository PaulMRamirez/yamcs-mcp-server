...
class TestLinkManagementComponent:
    ...
    async def test_link_list_links(self, links_component, mock_server, mock_yamcs_client):
        ...
        # Mock links
        mock_link1 = Mock()
        mock_link1.name = "TM_DOWN"
        mock_link1.status = "OK"
        mock_link1.class_name = "org.yamcs.tctm.TcpTmDataLink"
        mock_link1.enabled = True
        mock_link1.parent_name = None
        mock_link1.in_count = 1000
        mock_link1.out_count = 950

        mock_link2 = Mock(spec=['name', 'status'])
        mock_link2.name = "TC_UP"
        mock_link2.status = "FAILED"
        # Missing optional attributes - spec ensures getattr returns default
        ...
        assert result["links"][0]["type"] == "org.yamcs.tctm.TcpTmDataLink"
    ...
    async def test_link_get_status(self, links_component, mock_server, mock_yamcs_client):
        ...
        # Mock link info
        mock_link_info = Mock()
        mock_link_info.name = "TM_DOWN"
        mock_link_info.status = "OK"
        mock_link_info.class_name = "org.yamcs.tctm.TcpTmDataLink"
        mock_link_info.enabled = True
        mock_link_info.in_count = 5000
        mock_link_info.out_count = 4800
        mock_link_info.dataInTime = "2024-01-01T12:00:00Z"
        mock_link_info.dataOutTime = "2024-01-01T12:00:01Z"
        mock_link_info.detail_status = "Connected to ground station"
        
        # Mock link client
        mock_link_client = Mock()
        mock_link_client.get_info.return_value = mock_link_info

        mock_yamcs_client.get_link.return_value = mock_link_client
        ...
        assert result["type"] == "org.yamcs.tctm.TcpTmDataLink"
    ...
    async def test_link_get_statistics(self, links_component, mock_server, mock_yamcs_client):
        ...
        mock_link2 = Mock()
        mock_link2.name = "TC_UP"
        mock_link2.status = "OK"
        mock_link2.enabled = True
        mock_link2.in_count = 500
        mock_link2.out_count = 500
        
        mock_link3 = Mock()
        mock_link3.name = "BACKUP"
        mock_link3.status = "FAILED"
        mock_link3.enabled = False
        mock_link3.in_count = 0
        mock_link3.out_count = 0
        
        mock_links = [mock_link1, mock_link2, mock_link3]
    ...
    async def test_missing_attributes_handling(self, links_component, mock_server, mock_yamcs_client):
        ...
        mock_link_client = Mock()
        mock_link_client.get_info.return_value = mock_link
        mock_yamcs_client.get_link.return_value = mock_link_client
    ...
    async def test_resource_handlers(self, links_component, mock_server, mock_yamcs_client):
        ...
        mock_link2.name = "TC_UP"
        mock_link2.status = "FAILED"
        mock_link2.enabled = False
        
        mock_links = [mock_link1, mock_link2]
        # Set missing attributes on second link
        mock_links[1].class_name = None
        mock_links[1].in_count = None
        mock_links[1].out_count = None
        ...
        assert "TM_DOWN (org.yamcs.tctm.TcpTmDataLink): OK [in: 1000, out: 950]" in result
        assert "TC_UP" in result
        assert "DISABLED" in result
        # The actual output depends on whether the attributes are None or 0
    ...
    async def test_null_data_counts(self, links_component, mock_server, mock_yamcs_client):
        ...
        mock_link = Mock()
        mock_link.name = "NULL_COUNTS"
        mock_link.status = "OK"
        mock_link.enabled = True
        mock_link.in_count = None
        mock_link.out_count = None
